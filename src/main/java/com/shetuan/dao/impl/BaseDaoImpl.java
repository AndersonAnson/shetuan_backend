...
import com.shetuan.model.OrderEntity;
...
import com.shetuan.model.Filter.Operator;
import com.shetuan.model.Order.Direction;
...
public abstract class BaseDaoImpl<T, ID extends Serializable> implements BaseDao<T, ID> {
	...
	/**added by xyy  20171221  在CollectedController中首先被使用* */
	@SuppressWarnings("unchecked")
	@Override
	public Page<List<Object[]>> findPageBySqlAndParam(String jpql, Map<String, Object> params, Pageable pageable) {
		...
		long total = q.getResultList().size();
		List<Object[]> lst = q.setFlushMode(FlushModeType.COMMIT)
				.setFirstResult((pageable.getPage() - 1) * pageable.getRows()).setMaxResults(pageable.getRows())
				.getResultList();
		return new Page<List<Object[]>>(lst, total, pageable, 0);
	}
	
	/**added by xyy 20180130*/
	@SuppressWarnings("unchecked")
	@Override
	public Object getSingleResultByNativeQuery(String jpql){
		return  entityManager.createNativeQuery(jpql).setFlushMode(FlushModeType.COMMIT).getSingleResult();
	}
	
	/**added by xyy 20180131*/
	@SuppressWarnings("unchecked")
	@Override
	public List<Object[]> statis(String jpql){
		List<Object[]> list = entityManager.createNativeQuery(jpql).setFlushMode(FlushModeType.COMMIT).getResultList();
		return list;
	}
	...
	public Object deleteBySql(String jpql){
		Object num = entityManager.createNativeQuery(jpql).setFlushMode(FlushModeType.COMMIT).executeUpdate();
		return num;
	}
	...
	public Page<T> findPageJJ(Pageable pageable,T t) {
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<T> criteriaQuery = criteriaBuilder.createQuery(entityClass);
		criteriaQuery.select(criteriaQuery.from(entityClass));
		// modify by szy at 20160615
	...
	private List<Filter> getFiltersJJ(T t){
		List<Filter> filters = new ArrayList<Filter>();
		Field[] fields = t.getClass().getDeclaredFields(); // 获取对象的所有属性对象：type，name，value
		for (Field fd : fields) {
			String name = fd.getName();
...
//				restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.gt(root.<Number> get(filter.getProperty()), (Number) filter.getValue()));
				...
				else if(filter.getValue() instanceof Date)  //20180109 xyy 日期比较
